...
for i in os.listdir('C:/Users/moham/OneDrive/Bureau/summer_internship/data/grp0'):
    if len(os.listdir(os.path.join("C:/Users/moham/OneDrive/Bureau/summer_internship/data/grp0",i))):
        cab, cw, scale = [], [], []
        props_path=os.path.join("C:/Users/moham/OneDrive/Bureau/summer_internship/data/grp0",i,"props.json")
        f = open(props_path, 'r')
        props = js.load(f)
        for j in range(60):
        ...
        cab_patch, cw_patch = land_patch(positions, scale, cab, cw, np.zeros((120, 240)))
        patch = (np.zeros((2, 120, 240)).astype(np.float32))
        patch[0] = cab_patch
        patch[1] = cw_patch
        patch = torch.tensor(patch)
        torch.save(patch, os.path.join("C:/Users/moham/OneDrive/Bureau/summer_internship/data/grp0",i,'context.pt'))
        print(i)
